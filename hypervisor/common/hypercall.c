...
#define CONFIG_HV_RAM_SIZE 0x14000000

#ifndef CONFIG_RISCV_64
...
int32_t hcall_sos_offline_cpu(struct acrn_vm *vm, uint64_t lapicid)
{
	...
	foreach_vcpu(i, vm, vcpu) {
		if (vlapic_get_apicid(vcpu_vclint(vcpu)) == lapicid) {
 ...
 * @return 0 on success, non-zero on error.
 */
#define HV_API_MAJOR_VERSION 0
#define HV_API_MINOR_VERSION 0
...
#define DM_OWNED_GUEST_FLAG_MASK 0xff
				...
				const struct vm_memory_region *region,uint64_t *pml4_page)
{
	...
	if (hpa == INVALID_HPA) {
		...
		if (((hpa <= base_paddr) && ((hpa + region->size) > base_paddr)) ||
			...
			s2pt_add_mr(target_vm, pml4_page, hpa,
...
bool s2pt_is_mr_valid(const struct acrn_vm *vm, uint64_t base, uint64_t size)
{
	return true;
}
/**
	...
	struct acrn_vm *target_vm, const struct vm_memory_region *region)
{
	...
	if ((region->size & (PAGE_SIZE - 1UL)) != 0UL) {
		...
		if (!s2pt_is_mr_valid(target_vm, region->gpa, region->size)) {
			pr_err("%s, invalid gpa: 0x%lx, size: 0x%lx, top_address_space: 0x%lx", __func__,
				region->gpa, region->size,
				target_vm->arch_vm.s2pt_mem_ops.info->s2pt.top_address_space);
			ret = 0;
		} else {
...
/**
 * @brief setup s2pt memory mapping for multi regions
 *
 * @param vm Pointer to VM data structure
 * @param param guest physical address. This gpa points to
 *              struct set_memmaps
...
static int32_t write_protect_page(struct acrn_vm *vm,const struct wp_data *wp)
{
	...
	if ((!mem_aligned_check(wp->gpa, PAGE_SIZE)) ||
		(!s2pt_is_mr_valid(vm, wp->gpa, PAGE_SIZE))) {
		...
		if (hpa == INVALID_HPA) {
			...
			if (((hpa <= base_paddr) && ((hpa + PAGE_SIZE) > base_paddr)) ||
				...
				s2pt_modify_mr(vm, (uint64_t *)vm->arch_vm.s2ptp,
