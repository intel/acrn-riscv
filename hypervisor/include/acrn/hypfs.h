#ifndef __ACRN_HYPFS_H__
#define __ACRN_HYPFS_H__

#ifdef CONFIG_HYPFS
#include <acrn/list.h>
#include <acrn/string.h>
...
struct hypfs_entry {
    ...
    int (*read)(const struct hypfs_entry *entry,
                ACRN_GUEST_HANDLE_PARAM(void) uaddr);
    int (*write)(struct hypfs_entry_leaf *leaf,
                 ACRN_GUEST_HANDLE_PARAM(void) uaddr, unsigned int ulen);
};
...
#define HYPFS_DIR_INIT(var, nam)                  \
    struct hypfs_entry_dir __read_mostly var = {  \
        .e.type = ACRN_HYPFS_TYPE_DIR,             \
        .e.encoding = ACRN_HYPFS_ENC_PLAIN,        \
        .e.name = (nam),                          \
        .e.size = 0,                              \
        .e.max_size = 0,                          \
...
#define HYPFS_STRING_INIT(var, nam)               \
    HYPFS_VARSIZE_INIT(var, ACRN_HYPFS_TYPE_STRING, nam, 0)

/*
 * Set content and size of a ACRN_HYPFS_TYPE_STRING node. The node will point
...
#define HYPFS_UINT_INIT(var, nam, contvar)                       \
    HYPFS_FIXEDSIZE_INIT(var, ACRN_HYPFS_TYPE_UINT, nam, contvar, NULL)
#define HYPFS_UINT_INIT_WRITABLE(var, nam, contvar)              \
    HYPFS_FIXEDSIZE_INIT(var, ACRN_HYPFS_TYPE_UINT, nam, contvar, \
                         hypfs_write_leaf)

#define HYPFS_INT_INIT(var, nam, contvar)                        \
    HYPFS_FIXEDSIZE_INIT(var, ACRN_HYPFS_TYPE_INT, nam, contvar, NULL)
#define HYPFS_INT_INIT_WRITABLE(var, nam, contvar)               \
    HYPFS_FIXEDSIZE_INIT(var, ACRN_HYPFS_TYPE_INT, nam, contvar, \
                         hypfs_write_leaf)

#define HYPFS_BOOL_INIT(var, nam, contvar)                       \
    HYPFS_FIXEDSIZE_INIT(var, ACRN_HYPFS_TYPE_BOOL, nam, contvar, NULL)
#define HYPFS_BOOL_INIT_WRITABLE(var, nam, contvar)              \
    HYPFS_FIXEDSIZE_INIT(var, ACRN_HYPFS_TYPE_BOOL, nam, contvar, \
                         hypfs_write_bool)
