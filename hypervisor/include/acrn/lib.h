...
#include <acrn/inttypes.h>
#include <acrn/stdarg.h>
#include <acrn/types.h>
#include <acrn/xmalloc.h>
...
/* All clang versions supported by acrn have _Static_assert. */
...
#define _v(_x) ((long unsigned int)(_x))
...
#define gprintk(lvl, fmt, args...) \
    printk(ACRNLOG_GUEST lvl "%pv " fmt, current, ## args)
...
#define gdprintk(lvl, fmt, args...) \
    printk(ACRNLOG_GUEST lvl "%s:%d:%pv " fmt, \
           __FILE__, __LINE__, current, ## args)

#endif

/* vsprintf.c */
#define sprintf __acrn_has_no_sprintf__
#define vsprintf __acrn_has_no_vsprintf__
