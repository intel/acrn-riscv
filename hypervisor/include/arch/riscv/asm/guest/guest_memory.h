...
#ifndef __RISCV_GUEST_MEMORY_H__
#define __RISCV_GUEST_MEMORY_H__
 ...
 * VM related APIs
 */
extern int32_t gva2gpa(struct acrn_vcpu *vcpu, uint64_t gva, uint64_t *gpa, uint32_t *err_code);

extern enum vm_paging_mode get_vcpu_paging_mode(struct acrn_vcpu *vcpu);

/* gpa --> hpa -->hva */
extern void *gpa2hva(struct acrn_vm *vm, uint64_t x);
 ...
 */
extern int32_t copy_from_gpa(struct acrn_vm *vm, void *h_ptr, uint64_t gpa, uint32_t size);
/**
 ...
 * @pre Pointer vm is non-NULL
 */
extern int32_t copy_to_gpa(struct acrn_vm *vm, void *h_ptr, uint64_t gpa, uint32_t size);
/**
 ...
 */
extern int32_t copy_from_gva(struct acrn_vcpu *vcpu, void *h_ptr, uint64_t gva,
				uint32_t size, uint32_t *err_code, uint64_t *fault_addr);
#endif	/* !__ASSEMBLY__ */

#endif /* __RISCV_GUEST_MEMORY_H__ */
