...
uint32_t alloc_irq_vector(uint32_t irq)
{
	...
	if (irq < NR_IRQS) {
		...
		if (desc->arch.vector != VECTOR_INVALID) {
			if (vector_to_irq[desc->arch.vector] == irq) {
				/* statically binded */
				vr = desc->arch.vector;
			} else {
				pr_err("[%s] irq[%u]:vector[%u] mismatch",
					__func__, irq, desc->arch.vector);
				vr = VECTOR_INVALID;
			}
		} else {
...
static void free_irq_vector(uint32_t irq)
{
	...
	if (irq < NR_IRQS) {
		...
		if ((irq >= NR_LEGACY_IRQ) && (desc->arch.vector < VECTOR_FIXED_START)) {
			/* do nothing for LEGACY_IRQ and static allocated ones */

			spinlock_irqsave_obtain(&irq_alloc_spinlock, &rflags);
			vr = desc->arch.vector;
			desc->arch.vector = VECTOR_INVALID;
...
static inline bool irq_need_mask(const struct irq_desc *desc)
{
	...
	return (((desc->arch.flags & IRQF_LEVEL) != 0U)
...
static void init_irq_descs(void)
{
	...
	for (i = 0U; i < NR_IRQS; i++) {
		irq_desc[i].irq = i;
		irq_desc[i].arch.vector = VECTOR_INVALID;
