...
static int32_t stimer_vmexit_handler(struct acrn_vcpu *vcpu);
...
static const struct vm_exit_dispatch interrupt_dispatch_table[NR_HX_EXIT_IRQ_REASONS] = {
	...
	[HX_EXIT_IRQ_MSWI] = {
		.handler = undefined_vmexit_handler},
	[HX_EXIT_IRQ_STIMER] = {
		.handler = stimer_vmexit_handler},
	[HX_EXIT_IRQ_VIRT_STIMER] = {
...
int32_t vmexit_handler(struct acrn_vcpu *vcpu)
{
	...
	int32_t ret;
	struct vm_exit_dispatch *dispatch_table;

	if (get_processor_id() != pcpuid_from_vcpu(vcpu)) {
		    ...
		    (exit_type && basic_exit_reason >= ARRAY_SIZE(interrupt_dispatch_table)))
		{
			...
			if (dispatch->need_exit_qualification != 0U) {
				...
				vcpu->arch.exit_qualification = exec_vmread(HX_EXIT_QUALIFICATION);
			}
