...
DEFINE_PER_CPU(struct acrn_vcpu *, vcpu_array[CONFIG_MAX_VM_NUM]);
...
uint64_t vcpu_get_status(struct acrn_vcpu *vcpu)
{
	...
	if (!bitmap_test(CPU_REG_STATUS, &vcpu->reg_updated) &&
		...
		ctx->sstatus = exec_vmread(HX_GUEST_STATUS);
	}
...
{
	vcpu->arch.contexts[vcpu->arch.cur_context].run_ctx.sstatus =
		val;
...
static void vcpu_reset_internal(struct acrn_vcpu *vcpu, enum reset_mode mode)
{
	...
	vcpu->thread_obj.notify_mode = SCHED_NOTIFY_SWI;
...
DEFINE_PER_CPU(struct acrn_vcpu *, ever_run_vcpu);
...
void set_vcpu_regs(struct acrn_vcpu *vcpu, struct cpu_regs *vcpu_gp_regs)
{
	...
	vcpu_set_ip(vcpu, vcpu_gp_regs->ip);
...
static struct cpu_regs realmode_init_vregs = {
	.ip = 0xFFF0UL,
};

void reset_vcpu_gp_regs(struct acrn_vcpu *vcpu)
{
...
int32_t run_vcpu(struct acrn_vcpu *vcpu)
{
	...
	vcpu->arch.exit_reason = cpu_csr_read(scause);
	...
	if (status != 0) {
		...
		if ((vcpu->arch.exit_reason & HX_VMENTRY_FAIL) != 0U) {
			...
			pr_fatal("vmexit fail err_inst=%x", exec_vmread32(HX_INSTR_ERROR));
		}
	}
...
DECLARE_PER_CPU(struct sched_control, sched_ctl);
