...
{
	uint64_t guest_status, guest_state;
	bool status = false;

	/* Read the RFLAGS of the guest */
	guest_status = vcpu_get_status(vcpu);
	/* Check the RFLAGS[IF] bit first */
	if ((guest_status & HV_ARCH_VCPU_RFLAGS_IF) != 0UL) {
		/* Interrupts are allowed */
		/* Check for temporarily disabled interrupts */
		guest_state = exec_vmread32(HX_GUEST_INTERRUPTIBILITY_INFO);
	...
	if (vcpu == primary) {
		...
		if (vector <= NR_MAX_VECTOR) {
			dev_dbg(DBG_LEVEL_INTR, "VPIC: to inject PIC vector %d\n",
					vector & 0xFFU);
			exec_vmwrite32(HX_ENTRY_INT_INFO_FIELD,
					HX_INT_INFO_VALID |
					(vector & 0xFFU));
...
static bool vcpu_inject_exception(struct acrn_vcpu *vcpu)
{
	...
	if (bitmap_test_and_clear_lock(ACRN_REQUEST_EXCP, &vcpu->arch.pending_req)) {
		...
		if ((exception_type[vector] & EXCEPTION_ERROR_CODE_VALID) != 0U) {
			exec_vmwrite32(HX_ENTRY_EXCEPTION_ERROR_CODE,
					vcpu->arch.exception_info.error);
		}

		exec_vmwrite32(HX_ENTRY_INT_INFO_FIELD, HX_INT_INFO_VALID |
...
int32_t external_interrupt_vmexit_handler(struct acrn_vcpu *vcpu)
{
	...
	intr_info = exec_vmread32(HX_EXIT_INT_INFO);
	if (((intr_info & HX_INT_INFO_VALID) == 0U) ||
		...
		ctx.ip    = vcpu_get_ip(vcpu);
		ctx.status = vcpu_get_status(vcpu);
		...
		uint64_t *pending_req_bits, bool injected)
{
	...
	if (bitmap_test_and_clear_lock(ACRN_REQUEST_EVENT, pending_req_bits)) {
		vplic_inject_intr(vcpu_vlapic(vcpu), guest_irq_enabled, ret);
	}
}
...
int32_t exception_vmexit_handler(struct acrn_vcpu *vcpu)
{
	...
	intinfo = exec_vmread32(HX_EXIT_INT_INFO);
	if ((intinfo & HX_INT_INFO_VALID) != 0U) {
		 ...
		 */
		if ((intinfo & HX_INT_INFO_ERR_CODE_VALID) != 0U) {
			int_err_code = exec_vmread32(HX_EXIT_INT_ERROR_CODE);

			/* get current privilege level and fault address */
			cpl = exec_vmread32(HX_GUEST_CS_ATTR);
			cpl = (cpl >> 5U) & 3U;
