...
/*
 * pgentry may means vpn3/vpn2/vpn1
 */
static inline void construct_pgentry(uint64_t *vpn1, void *pd_page, uint64_t prot, const struct memory_ops *mem_ops)
{
	set_pgentry(vpn1, hva2hpa(pd_page) | (prot & ~(PAGE_R | PAGE_W | PAGE_X)), mem_ops);
...
{
	set_pgentry(ptep, paddr | prot, mem_ops);
		...
		uint64_t vaddr, const struct memory_ops *mem_ops)
{
	...
	for (i = 0UL; i < PTRS_PER_PTE; i++) {
		if (level == VPN1) {
			construct_pgentry(pbase + i, paddr, ref_prot, mem_ops);
		} else {
	...
	construct_pgentry(pte, pbase, ref_prot, mem_ops);
...
{
	uint64_t *pt_page = vpn1_page_vaddr(*vpn1);
...
static void modify_or_del_vpn1(const uint64_t *vpn2, uint64_t vaddr_start, uint64_t vaddr_end,
		uint64_t prot_set, uint64_t prot_clr, const struct memory_ops *mem_ops, uint32_t type)
{
	uint64_t *pd_page = vpn2_page_vaddr(*vpn2);
	uint64_t vaddr = vaddr_start;
...
static void modify_or_del_vpn2(const uint64_t *vpn3, uint64_t vaddr_start, uint64_t vaddr_end,
		uint64_t prot_set, uint64_t prot_clr, const struct memory_ops *mem_ops, uint32_t type)
{
	uint64_t *pdpt_page = vpn3_page_vaddr(*vpn3);
	uint64_t vaddr = vaddr_start;
	uint64_t index = vpn2_index(vaddr);

	pr_dbg("%s, vaddr: [0x%lx - 0x%lx]", __func__, vaddr, vaddr_end);
	for (; index < PTRS_PER_VPN2; index++) {
		uint64_t *vpn2 = pdpt_page + index;
		...
		uint64_t prot_set, uint64_t prot_clr, const struct memory_ops *mem_ops, uint32_t type)
{
	...
	while (vaddr < vaddr_end) {
		...
		vpn3 = vpn3_offset(pml4_page, vaddr);
		...
		uint64_t prot, const struct memory_ops *mem_ops)
{
	...
	for (; index < PTRS_PER_PTE; index++) {
		...
		if (mem_ops->pgentry_present(*pte) != 0UL) {
			...
			construct_pgentry(pte, paddr, prot, mem_ops);
		}
		paddr += PTE_SIZE;
		vaddr += PTE_SIZE;
		...
		if (vpn1_large(*vpn1) != 0UL) {
			...
			if (mem_ops->pgentry_present(*vpn1) == 0UL) {
				if (mem_ops->large_page_support(VPN1) &&
					...
					construct_pgentry(vpn1, pt_page, mem_ops->get_default_access_right(), mem_ops);
				}
			}
		...
		const struct memory_ops *mem_ops)
{
	...
	while (vaddr < vaddr_end) {
		...
		if (mem_ops->pgentry_present(*vpn3) == 0UL) {
			...
			construct_pgentry(vpn3, pdpt_page, mem_ops->get_default_access_right(), mem_ops);
		}
...
const uint64_t *lookup_address(uint64_t *pml4_page, uint64_t addr, uint64_t *pg_size, const struct memory_ops *mem_ops)
{
	...
	vpn3 = vpn3_offset(pml4_page, addr);
