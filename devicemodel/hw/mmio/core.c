...
int parse_pt_mmiodev(char *opt)
{
	...
	if (strncmp(opt, "pl011", 5) == 0) {
		strncpy(mmio_devs[0].name, "pl011", 5);
		err = 0;
	} else if((!dm_strtoul(opt, &cp, 16, &base_hpa) && *cp == ',') &&
...
{
	pr_err("init mmiodev hpa[%lx] gpa[%lx]\n\r", dev->base_hpa, dev->base_gpa);
...
struct mmio_dev_ops pl011 = {
	.name		= "pl011",
	.base_gpa	= 0xc0000000UL,
	.base_hpa	= 0x9000000UL,
	.size		= 0x00001000UL,
	.init		= init_pt_mmiodev,
	.deinit		= deinit_pt_mmiodev,
};
DEFINE_MMIO_DEV(pl011);
